...
export function AiFloatingHelper() {
  ...
  return (
    <section role="dialog" aria-label="Floating AI Helper" className="ai-chat-open fixed bottom-[calc(6.8rem+env(safe-area-inset-bottom))] right-3 z-[60] flex h-[min(500px,calc(100dvh-9rem))] w-[calc(100vw-1.5rem)] max-w-[380px] flex-col overflow-hidden rounded-[22px] border border-[#dbe4ff] bg-white shadow-[0_24px_64px_rgba(13,35,100,0.28)] lg:bottom-6 lg:right-6 lg:h-[500px]">
      ...
      <div className="border-t border-[#eef0f8] p-3">
        ...
        {attachmentError && <p className="mt-1 text-[10px] font-bold text-[#dc2626]">{attachmentError}</p>}
        <form onSubmit={submit} className="mt-2 flex gap-2"><button type="button" onClick={() => attachmentInputRef.current?.click()} disabled={loading || attachments.length >= AI_ATTACHMENT_MAX_COUNT} aria-label="Attach image or file" className="grid size-11 shrink-0 place-items-center rounded-xl border border-[#dfe3ef] bg-white text-[#59627a] disabled:opacity-40"><Paperclip size={17} /></button><input value={question} onChange={(event) => setQuestion(event.target.value)} aria-label="Floating AI question" placeholder="প্রশ্ন করুন..." className="min-w-0 flex-1 rounded-xl border border-[#dfe3ef] px-3 text-sm outline-none focus:border-[#6c4cf1]" /><button type="submit" disabled={loading || (!question.trim() && !attachments.length)} aria-label="Send floating AI question" className="grid size-11 shrink-0 place-items-center rounded-xl bg-[#6c4cf1] text-white disabled:opacity-50"><Send size={17} /></button></form>
        <Link href="/ai-helper" className="mt-2 flex items-center justify-center gap-1 text-[10px] font-extrabold text-[#11298f]"><MessageCircle size={12} /> Open full AI Helper</Link>
      </div>
    </section>
  );
}
