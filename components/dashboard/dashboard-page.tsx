  ...
  return (
    <div className="hidden lg:grid xl:grid-cols-[minmax(0,1fr)_248px] 2xl:grid-cols-[minmax(0,1fr)_280px]">
      ...
      <div className="grid gap-3 xl:grid-cols-[1.35fr_0.85fr]">
        <Card className="overflow-hidden rounded-xl border-[#dfe7e4] p-4 shadow-[0_5px_16px_rgba(35,58,49,0.05)]"><div className="mb-1 flex items-center justify-between"><div><h2 className="text-base font-black text-[#14231e]">Spending Trend</h2><p className="text-[10px] font-semibold text-[#7b8b85]">Daily expense movement</p></div><span className="rounded-lg border border-[#dfe7e4] px-2 py-1 text-[10px] font-extrabold text-[#62736d]">This Month</span></div><ExpenseTrendChart data={trendData} monthLabel={new Intl.DateTimeFormat("en-US", { month: "short" }).format(new Date(`${today.slice(0, 7)}-01T00:00:00`))} /><div className="flex justify-end"><Link href="/reports" className="flex items-center gap-1 text-[11px] font-black text-[#087d5a]">Full report <ChevronRight size={14} /></Link></div></Card>
        <Card className="rounded-xl border-[#dfe7e4] p-4 shadow-[0_5px_16px_rgba(35,58,49,0.05)]"><div className="mb-1 flex items-start justify-between"><div><h2 className="text-base font-black text-[#14231e]">Category Breakdown</h2><p className="text-[10px] font-semibold text-[#7b8b85]">This month distribution</p></div><Link href="/categories" className="grid size-8 place-items-center rounded-lg bg-[#edf6f2] text-[#087d5a]"><ArrowUpRight size={15} /></Link></div><CategoryPieChart data={categoryData} /><div className="grid gap-1.5">{topCategories.map((item) => <div key={item.name} className="flex items-center gap-2 text-[11px]"><span className="size-2 rounded-full" style={{ background: item.fill }} /><span className="min-w-0 flex-1 truncate font-bold text-[#52635d]">{item.name}</span><strong className="text-[#14231e]">{takaShort(item.value)}</strong></div>)}</div></Card>
      </div>
