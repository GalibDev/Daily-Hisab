    ...
    <div className="hidden lg:grid xl:grid-cols-[minmax(0,1fr)_280px]">
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {[
          ...
          <Card key={label} className="group relative min-h-[132px] overflow-hidden rounded-xl border-[#dfe7e4] p-4 shadow-[0_5px_16px_rgba(35,58,49,0.05)] transition hover:-translate-y-0.5 hover:shadow-[0_10px_24px_rgba(35,58,49,0.09)]"><div className="flex items-start justify-between gap-3"><span className={`grid size-9 place-items-center rounded-lg ${tone}`}><Icon size={18} /></span><ArrowUpRight size={15} className="text-[#a5b2ad] transition group-hover:text-[#087d5a]" /></div><p className="mt-3 text-[11px] font-bold text-[#65756f]">{label}</p><strong className="mt-1 block text-xl font-black tracking-[-0.03em] text-[#14231e]">{value}</strong><p className="mt-1 text-[10px] font-semibold text-[#899892]">{meta}</p></Card>
        ))}
      </div>

      <div className="grid gap-5 xl:grid-cols-[1.35fr_0.85fr]">
        <Card className="overflow-hidden border-[#e8ecf5] p-6 shadow-[0_14px_34px_rgba(20,35,90,0.06)]"><div className="mb-2 flex items-center justify-between"><h2 className="text-xl font-black text-[#111936]">Expense Trend</h2><span className="text-sm font-extrabold text-[#69718a]">This Month</span></div><ExpenseTrendChart data={trendData} monthLabel={new Intl.DateTimeFormat("en-US", { month: "short" }).format(new Date(`${today.slice(0, 7)}-01T00:00:00`))} /><div className="mt-2 flex justify-end"><Link href="/reports" className="flex items-center gap-1 text-xs font-black text-[#11298f]">Full report <ChevronRight size={15} /></Link></div></Card>
