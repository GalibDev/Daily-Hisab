  ...
  return (
    <div className="hidden lg:grid xl:grid-cols-[minmax(0,1fr)_248px] 2xl:grid-cols-[minmax(0,1fr)_280px]">
      ...
      <div className="grid gap-3 xl:grid-cols-[1.35fr_0.85fr]">
        <Card className="overflow-hidden rounded-xl border-[#dfe7e4] p-4 shadow-[0_5px_16px_rgba(35,58,49,0.05)]"><div className="mb-1 flex items-center justify-between"><div><h2 className="text-base font-black text-[#14231e]">{t("dashboard.spendingTrend")}</h2><p className="text-[10px] font-semibold text-[#7b8b85]">{t("dashboard.dailyMovement")}</p></div><span className="rounded-lg border border-[#dfe7e4] px-2 py-1 text-[10px] font-extrabold text-[#62736d]">{t("dashboard.thisMonth")}</span></div><ExpenseTrendChart data={trendData} monthLabel={new Intl.DateTimeFormat(language === "bangla" ? "bn-BD" : "en-US", { month: "short" }).format(new Date(`${today.slice(0, 7)}-01T00:00:00`))} /><div className="flex justify-end"><Link href="/reports" className="flex items-center gap-1 text-[11px] font-black text-[#087d5a]">{t("dashboard.fullReport")} <ChevronRight size={14} /></Link></div></Card>
        <Card className="rounded-xl border-[#dfe7e4] p-4 shadow-[0_5px_16px_rgba(35,58,49,0.05)]"><div className="mb-1 flex items-start justify-between"><div><h2 className="text-base font-black text-[#14231e]">{t("dashboard.categoryBreakdown")}</h2><p className="text-[10px] font-semibold text-[#7b8b85]">{t("dashboard.monthDistribution")}</p></div><Link href="/categories" className="grid size-8 place-items-center rounded-lg bg-[#edf6f2] text-[#087d5a]"><ArrowUpRight size={15} /></Link></div><CategoryPieChart data={categoryData} /><div className="grid gap-1.5">{topCategories.map((item) => <div key={item.name} className="flex items-center gap-2 text-[11px]"><span className="size-2 rounded-full" style={{ background: item.fill }} /><span className="min-w-0 flex-1 truncate font-bold text-[#52635d]">{item.name}</span><strong className="text-[#14231e]">{takaShort(item.value)}</strong></div>)}</div></Card>
      </div>

      <div className="grid gap-5 xl:grid-cols-[1.25fr_0.75fr]">
        <TodayEntries entries={entries} today={today} />
        <div className="grid gap-5"><BudgetOverviewCard /><Card className="border-[#e8ecf5] p-6 shadow-[0_14px_34px_rgba(20,35,90,0.06)]"><div className="mb-4 flex items-center justify-between"><div><p className="text-xs font-black uppercase tracking-[0.12em] text-[#7b84a0]">{t("dashboard.walletHealth")}</p><h2 className="mt-1 text-xl font-black text-[#111936]">{t("dashboard.moneyAtGlance")}</h2></div><ShieldCheck size={21} className="text-[#16a34a]" /></div><div className="grid gap-3"><div className="rounded-2xl bg-[#f3f6ff] p-4"><div className="flex items-center justify-between text-xs font-bold text-[#59627a]"><span>{t("dashboard.personalWallet")}</span><span>{t(wallet.personalEnabled ? "dashboard.active" : "dashboard.paused")}</span></div><strong className="mt-2 block text-2xl font-black text-[#11298f]">{takaShort(wallet.personalBalance)}</strong></div><div className="rounded-2xl bg-[#fff7ed] p-4"><div className="flex items-center justify-between text-xs font-bold text-[#8a5a22]"><span>{t("dashboard.familyWallet")}</span><span>{t(wallet.familyEnabled ? "dashboard.active" : "dashboard.paused")}</span></div><strong className="mt-2 block text-2xl font-black text-[#c2410c]">{takaShort(Math.max(0, combinedFamilyDeposits - wallet.familyExpenseTotal))}</strong></div></div><Link href="/hero-management" className="mt-4 flex items-center justify-between rounded-xl border border-[#e8ecf5] px-4 py-3 text-sm font-black text-[#11298f]">{t("dashboard.manageWallets")} <ArrowRight size={16} /></Link></Card></div>
      </div>

      <Card className="border-[#e8ecf5] p-6 shadow-[0_14px_34px_rgba(20,35,90,0.06)]"><div className="mb-4 flex items-center justify-between"><div><p className="text-xs font-black uppercase tracking-[0.12em] text-[#7b84a0]">{t("dashboard.activity")}</p><h2 className="mt-1 text-xl font-black text-[#111936]">{t("dashboard.recentMovement")}</h2></div><Link href="/entries" className="flex items-center gap-1 text-xs font-black text-[#11298f]">{t("dashboard.viewAll")} <ChevronRight size={15} /></Link></div>{latestEntries.length > 0 ? <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-5">{latestEntries.map((entry) => <div key={entry.id} className="rounded-2xl border border-[#edf0f6] bg-[#fbfcff] p-4"><div className="flex items-center justify-between gap-2"><span className="truncate text-xs font-black text-[#27304b]">{entry.category}</span><span className={entry.type === "income" ? "text-xs font-black text-[#16a34a]" : "text-xs font-black text-[#ef4444]"}>{entry.type === "income" ? "+" : "-"}{takaShort(entry.amount)}</span></div><p className="mt-2 truncate text-[11px] font-semibold text-[#7b849b]">{entry.description || t("dashboard.noDescription")}</p><p className="mt-2 text-[10px] font-bold text-[#a0a7b8]">{displayDate(entry.date)} · {entry.time}</p></div>)}</div> : <div className="rounded-2xl border border-dashed border-[#d9dfed] p-8 text-center text-sm font-semibold text-[#7a8298]">{t("dashboard.noRecentActivity")}</div>}</Card>
      {summaryRows.length > 0 && <div className="flex items-center gap-2 text-xs font-semibold text-[#7b849b]"><span className="size-2 rounded-full bg-[#16a34a]" /> {t("dashboard.latestSummary", { date: summaryRows[0].date })}</div>}
      </div>
...
export function DashboardPage() {
  ...
  return (
    <AppShell>
      <h1 className="sr-only">Daily Hisab (DailyHisab) – দৈনিক আয়-ব্যয়, বাজেট ও খরচের হিসাব অ্যাপ</h1>
