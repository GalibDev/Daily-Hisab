...
function MobileReportsAnalytics({
  ...
  return (
    <div className="grid gap-5 md:hidden">
      ...
      <section id="reports-filter" className="rounded-[18px] border border-[#e7eaf3] bg-white p-4 shadow-[0_10px_28px_rgba(20,35,90,0.06)]">
        <div className="mb-3 flex items-center justify-between gap-3">
          <div><h2 className="text-base font-extrabold text-[#111936]">Quick Filters</h2><p className="mt-0.5 text-[11px] font-semibold text-[#7a8298]">সব রিপোর্ট একসাথে আপডেট করুন</p></div>
          <span aria-live="polite" className="rounded-full bg-[#eef2ff] px-3 py-1 text-[10px] font-extrabold text-[#11298f]">{filterLabel}</span>
        </div>
        <div className="grid grid-cols-4 gap-2">
          {([
            ...
            ["thisYear", "This Year"],
            ["custom", "Custom"],
          ] as Array<[AnalyticsFilter, string]>).map(([key, item]) => <button key={key} type="button" aria-pressed={analyticsFilter === key} aria-label={`Filter reports by ${item}`} onClick={() => setAnalyticsFilter(key)} className={analyticsFilter === key ? "min-h-12 rounded-xl bg-[#11298f] px-1 text-[10px] font-extrabold text-white shadow-[0_8px_18px_rgba(17,41,143,0.20)]" : "min-h-12 rounded-xl border border-[#e4e8f2] bg-[#fbfcff] px-1 text-[10px] font-extrabold text-[#111936]"}><CalendarDays size={15} aria-hidden="true" className="mx-auto mb-1" />{item}</button>)}
        </div>
...
export function PrivacyPolicyPage() {
  const sections = [
    { title: "Information we handle", text: "Daily Hisab stores the account information you provide, such as your name, email address and profile photo, together with expense, income, budget, loan, reminder, note and category records you create." },
    { title: "Local and cloud data", text: "Some preferences and offline records may be saved in your browser or device. When cloud sync is enabled, your financial records are stored with Firebase under your authenticated account so they can appear on your other signed-in devices." },
    { title: "Photos, receipts and files", text: "Profile photos, receipt images and backup files are processed only when you choose to upload or export them. Daily Hisab does not access unrelated photos or files on your device." },
    { title: "How information is used", text: "Your information is used to provide calculations, reports, reminders, backup, synchronization and other features you request. We do not sell your personal or financial information." },
    { title: "Security and control", text: "Authentication and cloud access are protected through Firebase security controls. You remain responsible for protecting your password and device. You may export, edit or delete your records using the available account and data-management tools." },
    { title: "Third-party services", text: "Daily Hisab may rely on Firebase, Vercel and optional AI or Google services to provide authentication, hosting, synchronization and selected features. Those services may process limited technical information under their own privacy terms." },
  ];
  return <AppShell><PageTitle title="Privacy Policy" subtitle="How Daily Hisab handles and protects your information" /><div className="mx-auto grid max-w-3xl gap-5"><section className="overflow-hidden rounded-[26px] bg-gradient-to-br from-[#071b75] via-[#11298f] to-[#2478d6] p-6 text-white shadow-[0_22px_55px_rgba(17,41,143,0.24)] sm:p-8"><span className="grid size-14 place-items-center rounded-2xl bg-white/14 ring-1 ring-white/20"><ShieldCheck size={29} /></span><h1 className="mt-5 text-2xl font-black">Your privacy matters</h1><p className="mt-2 text-sm font-semibold leading-6 text-white/78">This policy explains what information Daily Hisab uses, where it is stored and the choices available to you.</p><p className="mt-4 text-xs font-bold text-white/65">Effective: 7 August 2026 · Version 7.0</p></section><Card className="rounded-[24px] border-[#e6ebf5] p-5 shadow-[0_16px_42px_rgba(20,35,90,0.07)] sm:p-7"><div className="grid gap-6">{sections.map((section, index) => <section key={section.title} className="grid grid-cols-[36px_1fr] gap-3"><span className="grid size-9 place-items-center rounded-xl bg-[#eef4ff] text-xs font-black text-[#11298f]">{index + 1}</span><div><h2 className="font-extrabold text-[#111936]">{section.title}</h2><p className="mt-2 text-sm font-semibold leading-6 text-[#69718a]">{section.text}</p></div></section>)}</div><div className="mt-7 rounded-2xl border border-[#dce5ff] bg-[#f5f8ff] p-4"><h2 className="font-extrabold text-[#111936]">Privacy questions</h2><p className="mt-2 text-sm font-semibold leading-6 text-[#69718a]">For privacy requests or questions, contact developer Mirza Galib Palash.</p><a href="mailto:mirza.galib.palash@gmail.com?subject=Daily%20Hisab%20Privacy" className="mt-3 inline-flex items-center gap-2 text-sm font-extrabold text-[#11298f]"><Mail size={17} /> mirza.galib.palash@gmail.com</a></div></Card></div></AppShell>;
}
